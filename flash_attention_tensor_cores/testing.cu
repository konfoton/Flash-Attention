...
int main(){
    // test parameters (keep current template size in testing.cu)
    const int B = 1;
    const int H = 1;
    const int L =  64;
    const int D = 128;
    ...
    // host half buffers (wrapper manages device allocations/copies)
    std::vector<__half> Q_half(B * H * L * D);
    std::vector<__half> K_half(B * H * L * D);
    std::vector<__half> V_half(B * H * L * D);
    std::vector<__half> O_half(B * H * L * D);
    ...
    // wrapper will handle device copies


    // hyperparameters
    const int tile = 64; // keep kernel tile size consistent
    ...
    // Use wrapper to run with host half buffers
    float elapsed_ms = 0.0f;
    run_tensor_flash_attention_host_half(
        Q_half.data(), K_half.data(), V_half.data(), O_half.data(),
        1, H, L, D, tile,
        0, &elapsed_ms);
    ...
    for(size_t i = 0; i < O_half.size(); ++i){
        O_gpu[i] = __half2float(O_half[i]);
    }
