...
#include <cuda_fp16.h>
#include <stdexcept>
#include "cuda_flash_attention.cuh"
#include "wrapper.cuh"

extern "C" void run_cuda_cores_flash_attention_host_half(
    const __half* hQ,
    const __half* hK,
    const __half* hV,
    __half* hO,
    ...
    float* elapsed_ms)
{
    ...
    if (cudaMalloc(&dQ, bytes) != cudaSuccess ||
        ...
        throw std::runtime_error("cudaMalloc failed in run_cuda_cores_flash_attention_host_half");
    }
    ...
    run_flash_attention(dQ, dK, dV, dO, B, H, L, D, KQVO_block_y, number_of_warps_per_block, stream, elapsed_ms);
